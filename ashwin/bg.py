...
class scrolling_bg():
    def __init__(self, DISPLAY, bg_color, image_paths, objects, spin=False):
        ...
        for n in range(objects):
            self.objects_a.append([random.randint(0,1), [random.randint(15,self.display.get_width() - 15),random.randint(15, self.display.get_height() - 15)], random.randint(4,20)/10])
